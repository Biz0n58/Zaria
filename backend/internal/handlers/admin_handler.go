...
func (h *AdminHandler) GetOrders(c *fiber.Ctx) error {
	...
	query := `SELECT id, user_id, customer_email, status, subtotal_cents, shipping_cents, total_cents, currency, created_at, updated_at 
	...
	for rows.Next() {
		var o models.Order
		err := rows.Scan(
			&o.ID, &o.UserID, &o.CustomerEmail, &o.Status, &o.SubtotalCents,
