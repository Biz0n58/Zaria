...
func (h *PaymentHandler) CreateStripeIntent(c *fiber.Ctx) error {
	...
	pi, err := services.CreatePaymentIntent(int64(order.TotalCents), order.Currency, order.ID.String())
	if err != nil {
...
func (h *PaymentHandler) StripeWebhook(c *fiber.Ctx) error {
	...
	if endpointSecret == "" {
		return c.Status(400).JSON(fiber.Map{"error": "webhook secret not configured"})
	}
